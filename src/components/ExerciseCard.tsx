...
export default function ExerciseCard({ exercise, showDetailsButton = true, onClick }: ExerciseCardProps) {
    return (
        <div
            onClick={onClick}
            className={`flex items-center gap-3 bg-[var(--surface)] rounded-[var(--radius-lg)] p-4 transition-all duration-200 ${
                onClick ? "cursor-pointer active:scale-[0.99]" : ""
            }`}
        >
            {exercise.is_custom ? (
                <div className="flex-shrink-0 w-14 h-14 rounded-[var(--radius-sm)] bg-[var(--surface-raised)] flex items-center justify-center">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    <h3 className="text-sm sm:text-base font-semibold text-[var(--foreground)] truncate" style={{ fontFamily: "var(--font-poppins)" }}>
            ...
            {showDetailsButton && (
                <Link href={`/exercises/${exercise.exercise_id}`} className="flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                    <Button variant="primary" className="px-3 py-1.5 text-xs font-semibold whitespace-nowrap">
                        Details
                    </Button>
                </Link>
            )}
