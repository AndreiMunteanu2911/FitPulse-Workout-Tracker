...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <ul className="flex flex-col gap-2 flex-1 overflow-y-auto">
          {tabs.map((tab) => (
            <li key={tab.name}>
              <Link
                href={tab.href}
                className={"flex items-center px-4 py-2 rounded-md transition hover:bg-white/10 " +
      ...
      <nav className="fixed bottom-0 left-0 right-0 md:hidden bg-[var(--primary-800)] text-white z-20 border-t border-[var(--primary-900)]">
