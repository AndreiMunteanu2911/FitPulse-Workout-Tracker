...
export default function ExerciseListCard({ exercise, onEdit, onDelete }: ExerciseListCardProps) {
  return (
    <div className="bg-[var(--surface)] rounded-[var(--radius-xl)] shadow-[var(--shadow)] p-4 sm:p-5 flex items-center gap-4 hover:shadow-[var(--shadow-md)] hover:-translate-y-0.5 transition-all duration-200">
      <div className="flex-1 min-w-0">
        <h3 className="text-base font-bold text-[var(--foreground)] truncate">{exercise.name}</h3>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          ...
          className="w-9 h-9 rounded-lg bg-[var(--surface-raised)] hover:bg-[var(--primary-50)] dark:hover:bg-[var(--primary-100)] flex items-center justify-center transition-colors"
        >
          <Pencil className="w-4 h-4 text-[var(--muted-foreground)] hover:text-[var(--primary-500)]" />
        </button>
        <button
          onClick={() => onDelete(exercise)}
          className="w-9 h-9 rounded-lg bg-[var(--surface-raised)] hover:bg-[var(--color-destructive-bg)] flex items-center justify-center transition-colors"
        >
