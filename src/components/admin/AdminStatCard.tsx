...
export default function AdminStatCard({ title, value, subtitle, icon, accentColor = "bg-[var(--primary-500)]" }: AdminStatCardProps) {
  return (
    <div className="bg-[var(--surface)] rounded-[var(--radius-xl)] p-5 shadow-[var(--shadow)] relative overflow-hidden">
      <div className={`absolute left-0 inset-y-0 w-1 ${accentColor}`} />
      {icon && (
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-[var(--muted-foreground)]">{icon}</span>
          <p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted-foreground)]">{title}</p>
        </div>
      )}
      {!icon && (
